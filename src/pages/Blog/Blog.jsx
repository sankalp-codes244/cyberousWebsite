...
const Blog = () => {
  ...
  const blogData = [
    {
      id: 1,
      name: "How to Build a Career in Cybersecurity in 2024?",
      desc: "In an increasingly digital world fraught with evolving threats, cybersecurity has emerged as a critical profession, safeguarding organizations, individuals, and nations against malicious cyber activities. A cybersecurity career encompasses a wide array of roles, each contributing to the overarching goal of protecting digital assets, maintaining privacy, and ensuring the integrity of systems and networks.",
      image: 'https://cdn.hashnode.com/res/hashnode/image/upload/v1712068632891/773f81fe-1d9c-4b44-8014-3710e4f8cd4b.png?w=1600&h=840&fit=crop&crop=entropy&auto=compress,format&format=webp',
      navigation: 'https://cyberous.hashnode.dev/how-to-build-a-career-in-cybersecurity-in-2024'
    },
  ];
  const data = [
    {
  ...
  return (
    <div className='h-full'>
      {/* Background section */}
      <div className='text-center relative bg-no-repeat bg-cover bg-center h-screen object-cover' style={{ backgroundImage: `url(${Images.BLOGBG})`, }}>
        <div className='h-full bg-black bg-opacity-50 flex flex-col justify-center'>
          <h1 className='text-2xl sm:text-3xl lg:text-5xl font-__Inter_aaf875 font-bold text-white'>Our Blogs</h1>
          <div className="h-1 w-32 bg-custom-buttonColor-Green my-4 mx-auto  rounded-full"></div>
          <div className='w-full lg:px-72'>

          <h2 className='text-xl sm:text-xl lg:text-2xl text-white font-bold font-__Inter_aaf875'>Explore the latest Whitepapers from our experts to keep upto date about the latest cyber security trends.</h2>
          </div>
        </div>
      </div>
      ...
      <div className='px-4 lg:px-32 py-8'>
        <div className='grid grid-cols-1 lg:grid-cols-3 gap-6'>
         ...
         <BlogComponent key={index} desc={item.desc} image={item.image} name={item.name} navigation={item.navigation} />
          ))}
